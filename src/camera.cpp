...
CCamera* StartCamera(int width, int height, int framerate, int num_levels, bool do_argb_conversion)
{
	...
	if(GCamera != NULL)
	{
		syslog(LOG_NOTICE, "Can't create more than one camera\n");
		return NULL;
	}
	...
	{
		//failed so clean up
		syslog(LOG_ERR, "Camera init failed\n");
		delete GCamera;
		GCamera = NULL;
	}
	return GCamera;
...
MMAL_COMPONENT_T* CCamera::CreateCameraComponentAndSetupPorts()
{
	...
	{
		syslog(LOG_ERR, "Failed to create camera component\n");
		return NULL;
	}
	...
	{
		syslog(LOG_ERR, "Camera doesn't have output ports");
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Unable to enable control port : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Couldn't set preview port format : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Couldn't set video port format : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Couldn't set still port format : error %d", status);
		mmal_component_destroy(camera);
		return NULL;
	}
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Couldn't enable camera\n");
		mmal_component_destroy(camera);
		return NULL;
	}
...
MMAL_COMPONENT_T* CCamera::CreateSplitterComponentAndSetupPorts(MMAL_PORT_T* video_output_port)
{
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Failed to create splitter component\n");
		goto error;
	}
	...
	{
		syslog(LOG_ERR, "Splitter doesn't have correct ports: %d, %d\n",splitter->input_num,splitter->output_num);
		goto error;
	}
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Couldn't set resizer input port format : error %d", status);
		goto error;
	}
	...
	for(int i = 0; i < splitter->output_num; i++)
	{
		...
		if (status != MMAL_SUCCESS)
		{
			syslog(LOG_ERR, "Couldn't set resizer output port format : error %d", status);
			goto error;
		}
	}
...
bool CCamera::Init(int width, int height, int framerate, int num_levels, bool do_argb_conversion)
{
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Failed to enable connection\n");
		goto error;
	}
	...
	for(int i = 0; i < num_levels; i++)
	{
		...
		{
			syslog(LOG_ERR, "Failed to initialize output %d\n",i);
			goto error;
		}
	}
	...
	{
		syslog(LOG_ERR, "Failed to start capture\n");
		goto error;
	}
	...
	//return success
	syslog(LOG_NOTICE, "Camera successfully created\n");
	return true;
...
CCameraOutput* CCamera::getOutput(int level) {
...
{
	syslog(LOG_NOTICE, "Init camera output with %d/%d\n",width,height);
	Width = width;
	Height = height;
	...
	if(!output_queue)
	{
		syslog(LOG_ERR, "Failed to create output queue\n");
		goto error;
	}
		...
		if(MMAL_BUFFER_HEADER_T* existing_buffer = mmal_queue_get(OutputQueue))
		{
			...
			if (port->is_enabled)
			{
				...
				if (!new_buffer || status != MMAL_SUCCESS)
					syslog(LOG_ERR, "Unable to return a buffer to the video port\n");
			}
		}
	}
...
MMAL_COMPONENT_T* CCameraOutput::CreateResizeComponentAndSetupPorts(MMAL_PORT_T* video_output_port, bool do_argb_conversion)
{
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Failed to create reszie component\n");
		goto error;
	}
	...
	{
		syslog(LOG_ERR, "Resizer doesn't have correct ports");
		goto error;
	}
...
MMAL_POOL_T* CCameraOutput::EnablePortCallbackAndCreateBufferPool(MMAL_PORT_T* port, MMAL_PORT_BH_CB_T cb, int buffer_count)
{
	...
	syslog(LOG_NOTICE, "Creating pool with %d buffers of size %d\n", port->buffer_num, port->buffer_size);
	buffer_pool = mmal_port_pool_create(port, port->buffer_num, port->buffer_size);
	if (!buffer_pool)
	{
		syslog(LOG_ERR, "Couldn't create video buffer pool\n");
		goto error;
	}
	...
	if (status != MMAL_SUCCESS)
	{
		syslog(LOG_ERR, "Failed to set video buffer callback\n");
		goto error;
	}
	...
	//send all the buffers in our pool to the video port ready for use
	{
		...
		for (q=0;q<num;q++)
		{
			...
			if (!buffer)
			{
				syslog(LOG_ERR, "Unable to get a required buffer %d from pool queue\n", q);
				goto error;
			}
			else if (mmal_port_send_buffer(port, buffer)!= MMAL_SUCCESS)
			{
				syslog(LOG_ERR, "Unable to send a buffer to port (%d)\n", q);
				goto error;
			}
		}
	}
